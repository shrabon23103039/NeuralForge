...
- Name: home.spec.ts >> Home Page & Map View >> should toggle category filters on map view
- Location: e2e\home.spec.ts:29:7
...
Locator: getByRole('button', { name: /crime only/i })
Expected pattern: /bg-rose-600/
Received string:  "px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors bg-slate-800 text-slate-300 hover:bg-slate-700"
Timeout: 5000ms

Call log:
  - Expect "toHaveClass" with timeout 5000ms
  - waiting for getByRole('button', { name: /crime only/i })
    13 × locator resolved to <button class="px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors bg-slate-800 text-slate-300 hover:bg-slate-700">…</button>
       - unexpected value "px-3 py-1.5 rounded-lg text-xs font-semibold transition-colors bg-slate-800 text-slate-300 hover:bg-slate-700"
