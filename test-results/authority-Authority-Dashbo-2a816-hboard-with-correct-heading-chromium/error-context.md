...
- Name: authority.spec.ts >> Authority Dashboard Workflow >> should display authority dashboard with correct heading
- Location: e2e\authority.spec.ts:15:7
...
Locator: getByRole('heading', { name: /authority triage dashboard/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: /authority triage dashboard/i })
...
```ts
  ...
  2   | 
  3   | test.describe('Authority Dashboard Workflow', () => {
  ...
  13  |   });
  14  | 
  15  |   test('should display authority dashboard with correct heading', async ({ page }) => {
  16  |     await page.goto('/authority');
  17  | 
  18  |     // Check heading: t('authority_title') = "Authority Triage Dashboard"
> 19  |     await expect(page.getByRole('heading', { name: /authority triage dashboard/i })).toBeVisible();
      |                                                                                      ^ Error: expect(locator).toBeVisible() failed
  20  | 
