...
- Name: auth.spec.ts >> Authentication Workflow >> should show role selection in signup mode
...
Locator: getByRole('button', { name: /citizen/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /citizen/i }).first()
